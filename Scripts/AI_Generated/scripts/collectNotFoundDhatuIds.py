...
Location: Scripts/AI_Generated/scripts/collectNotFoundDhatuIds.py
...
def collect_not_found_dhatu_ids(json_file):
    ...
    not_found_cases_with_gati = OrderedDict()
    total_cases = 0
                    ...
                    for entry in verb_block.get('entries', []):
                        ...
                        if dhatu_id == "Not Found":
                            ...
                            if gati and gati.strip():
                                ...
                                not_found_cases_with_gati[key] = entry_data
                                with_gati_count += 1
                                print(f"Found (with gati): {key}")
                            else:
                                # No gati
                                key = form
    ...
    print(f"\n{'='*60}")
    print(f"✅ Found {total_cases} cases with 'Not Found' dhatu_id")
    print(f"  - Without gati: {without_gati_count}")
    print(f"  - With gati: {with_gati_count}")
    print(f"{'='*60}")

    return not_found_cases_without_gati, not_found_cases_with_gati
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 Scripts/AI_Generated/scripts/collectNotFoundDhatuIds.py <json_file> <output_dir>")
        print("\nExample (from project root):")
        print("  python3 Scripts/AI_Generated/scripts/collectNotFoundDhatuIds.py \\")
        ...
        print("  - Scripts/AI_Generated/output/not_found_dhatu_ids_with_gati.yaml")
        sys.exit(1)
    ...
    output_with_gati = os.path.join(output_dir, "not_found_dhatu_ids_with_gati.yaml")

    print(f"📚 Loading JSON from {json_file}...")
    cases_without_gati, cases_with_gati = collect_not_found_dhatu_ids(json_file)
